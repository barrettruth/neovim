...
static int char_iter_next(const uint8_t **itp, int nomap)
{
  ...
  if (nomap == 0 && mblen > 1) {
    ...
    const char *mb = mb_unescape(&p);
    if (mb != NULL) {
      *itp = (const uint8_t *)p;
      return utf_ptr2char(mb);
    }

    const uint8_t *probe = it;
    for (int i = 0; i < mblen; i++) {
      if (*probe == K_SPECIAL) {
        // Follows vgetc()'s logic:
        // Must be a K_SPECIAL - KS_SPECIAL - KE_FILLER sequence,
        // which represents a K_SPECIAL (0x80).
        // skip KS_SPECIAL and KE_FILLER
...
static int handle_mapping(int *keylenp, const bool *timedout, int *mapdepth)
{
  ...
  if (no_mapping == 0
    ...
    if (tb_b1 == K_SPECIAL) {
      ...
      tb_c1 = char_iter_next(&tb_iter, 0);
    ...
    for (; mp != NULL; mp->m_next == NULL ? (mp = mp2, mp2 = NULL) : (mp = mp->m_next)) {
      ...
      if (char_iter_next(&key_iter, 0) == tb_c1 && (mp->m_mode & local_State)
        ...
        int mlen = (int)(key_iter - (const uint8_t *)mp->m_keys);  // Match length in key buffer
        int tb_mlen = (int)(tmp_tb_iter - typebuf.tb_buf) - typebuf.tb_off;  // Match length in typeahead buffer

        // find the match length of this mapping
        while ((c2 = char_iter_next(&tmp_tb_iter, nomap)) != 0) {
          int tb_nomap = nomap;
          ...
          int key_char = char_iter_next(&key_iter, tb_nomap);
